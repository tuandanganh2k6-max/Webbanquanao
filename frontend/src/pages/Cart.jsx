        ...
        {cartItems.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-16 flex flex-col items-center justify-center text-center">
            <div className="w-24 h-24 bg-slate-100 rounded-full flex items-center justify-center mb-6">
              <svg className="w-12 h-12 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
            </div>
                  ...
                  {cartItems.map((item) => (
                    <li key={item.cartId} className="p-6 flex flex-col sm:flex-row items-center gap-6 group hover:bg-slate-50 transition-colors">
                      ...
                      <div className="flex-1 w-full text-center sm:text-left">
                        ...
                        <p className="text-sm text-slate-500 mb-2">Phân loại: <span className="font-semibold text-slate-700">{item.color}</span> | Kích cỡ: <span className="font-semibold text-slate-700">{item.size}</span></p>
                        <p className="text-lg font-black text-blue-600">{item.price.toLocaleString('vi-VN')} đ</p>
                      </div>
                      <div className="flex items-center gap-4 mt-4 sm:mt-0">
                        <div className="px-4 py-2 bg-slate-100 rounded-lg text-slate-700 font-bold">SL: {item.qty}</div>
                        <button 
