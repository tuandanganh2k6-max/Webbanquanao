      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row gap-12 bg-white rounded-[3rem] p-8 lg:p-12 shadow-sm border border-slate-100">
          ...
          <div className="md:w-1/2 flex flex-col justify-center">
            <div className="mb-4">
              <span className="inline-block px-4 py-1.5 bg-blue-50 text-blue-600 text-[10px] font-black rounded-full uppercase tracking-widest">
                {product.brand || 'T-SHOP EXCLUSIVE'}
              </span>
            </div>
            <h1 className="text-4xl sm:text-5xl font-black text-slate-900 mb-6 leading-none tracking-tighter">{product.name}</h1>
            <p className="text-4xl text-blue-600 font-black mb-8 tracking-tighter">
              {product.price.toLocaleString('vi-VN')} <span className="text-xl">đ</span>
            </p>
                ...
                <div className="flex flex-wrap gap-3">
                  {product.colors.map((color) => (
                    <button
                      ...
                      className={`px-6 py-2.5 rounded-full border-2 text-sm font-black transition-all ${selectedColor === color ? 'border-blue-600 bg-blue-50 text-blue-600 shadow-md' : 'border-slate-100 text-slate-400 hover:border-slate-200 hover:text-slate-600'}`}
                    >
                      {color}
                    </button>
                  ))}
              ...
              <div className="flex flex-wrap gap-3">
                {availableSizes.map((size) => (
                  <button
                    ...
                    className={`w-14 h-14 rounded-2xl border-2 flex items-center justify-center text-sm font-black transition-all ${selectedSize === size ? 'border-blue-600 bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'border-slate-100 text-slate-400 hover:border-slate-200 hover:text-slate-600'}`}
                  >
                    {size}
                  </button>
                ))}
